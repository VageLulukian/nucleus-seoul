...
// Раунд 2 (consult Codex, оператор забраковал кальку): «ЛОКАЦИЯ СОВПАДАЕТ С ПРОГНОЗОМ» →
// «ПРОГНОЗ: БЕЗ РАСХОЖДЕНИЙ» («локация» — не казёнщина, а жаргон; «совпадает с прогнозом» —
// калька). «ВНЕ ЗОНЫ ОТВЕТСТВЕННОСТИ» → «НЕ УЧИТЫВАЛСЯ»: система не считала чувства героя
// параметром (точнее добивает, тон §7 цел — не злая и не тупая). Короче под узкую колонку.
export const FINAL = {
  arrival: { label: 'ПРИБЫТИЕ ЗАФИКСИРОВАНО:', value: 'СЕУЛ' },   // журнал прибытия
  match: 'ПРОГНОЗ: БЕЗ РАСХОЖДЕНИЙ',                              // система холодно констатирует свою правоту
  comfort: { label: 'КОМФОРТ ПОЛЬЗОВАТЕЛЯ:', value: 'НЕ УЧИТЫВАЛСЯ' }, // добивает: чувства не были параметром
};
