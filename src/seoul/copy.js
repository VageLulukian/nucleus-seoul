...
// аудитория русская — холодный казённый журнал на русском, чтобы шутка читалась зрителем.
// Регистр: канцелярит, uppercase, без эмоций. «СЕУЛ» — кириллицей.
// Раунд 3 (направление оператора + разгон с Codex): абстрактные метрики («прогноз/
// расхождение/параметр») — нечитаемый жаргон, выкинуты. Стало — конкретная метео-сводка
// прибытия: СЕУЛ → ПОГОДА: ДОЖДЬ · СЕЗОН ДОЖДЕЙ (реальный 장마, конец июня) → КОМФОРТ
// ПОЛЬЗОВАТЕЛЯ: ВНЕ ЗАДАЧИ. Связь читается за секунду: система знает про дождь и холодно
// сообщает, что комфорт героя — не её забота. Тон §7 цел: не тупая (дождь знает), не интриган.
// match — ключ исторический; теперь держит метео-строку (середина), не «совпадение с прогнозом».
export const FINAL = {
  arrival: { label: 'ПРИБЫТИЕ ЗАФИКСИРОВАНО:', value: 'СЕУЛ' },   // журнал прибытия
  match: 'ПОГОДА: ДОЖДЬ · СЕЗОН ДОЖДЕЙ',                          // конкретное бытовое наказание — мокрый Сеул
  comfort: { label: 'КОМФОРТ ПОЛЬЗОВАТЕЛЯ:', value: 'ВНЕ ЗАДАЧИ' }, // холодный дисклеймер: чувства — не её забота
};
